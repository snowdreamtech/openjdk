...
docker buildx build --platform=linux/386,linux/amd64,linux/arm/v6,linux/arm/v7,linux/arm64,linux/ppc64le,linux/riscv64,linux/s390x \
    ...
    -t ${DOCKER_HUB_PROJECT}:8.402.06 \
    -t ${DOCKER_HUB_PROJECT}:8.402 \
    -t ${DOCKER_HUB_PROJECT}:8 \
    -t ${GITHUB_PROJECT}:latest \
    -t ${GITHUB_PROJECT}:8.402.06 \
    -t ${GITHUB_PROJECT}:8.402 \
    -t ${GITHUB_PROJECT}:8 \
    -t ${DOCKER_HUB_PROJECT1}:latest \
    -t ${DOCKER_HUB_PROJECT1}:8.402.06 \
    -t ${DOCKER_HUB_PROJECT1}:8.402 \
    -t ${DOCKER_HUB_PROJECT1}:8 \
    -t ${GITHUB_PROJECT1}:latest \
    -t ${GITHUB_PROJECT1}:8.402.06 \
    -t ${GITHUB_PROJECT1}:8.402 \
    -t ${GITHUB_PROJECT1}:8 \
    . \
    --push
